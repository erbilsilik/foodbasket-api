(function ($) {
  ...
  DayScheduleSelector.prototype.renderRows = function () {
    ...
    $.each(generateDates(start, end, interval), function (i, d) {
      var daysInARow = $.map(new Array(days.length), function (_, i) {
        return '<td class="time-slot" data-time="' + hhmm(d) + '" data-day="' + days[i] + '"><input style="display: none" id="' + hhmm(d) + '-' + days[i] + '" type="checkbox" name="days_hours[]"  value="' + hhmm(d) + '-' + days[i] + '"></td></label>'
      }).join();
  ...
  DayScheduleSelector.prototype.deselect = function ($slot) { $slot.removeAttr('data-selected');  $slot.find('input[type=checkbox]').prop("checked", false);}
